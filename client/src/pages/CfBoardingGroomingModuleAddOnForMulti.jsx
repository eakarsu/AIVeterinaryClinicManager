...
// Feature: Boarding/grooming module add-on for multi-service clinics
...
export default function CfBoardingGroomingModuleAddOnForMulti() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-boarding-grooming-module-add-on-for-multi-service-clinics/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-boarding-grooming-module-add-on-for-multi-service-clinics', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-boarding-grooming-module-add-on-for-multi-service-clinics/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Boarding/grooming module add-on for multi-service clinics</h1>
