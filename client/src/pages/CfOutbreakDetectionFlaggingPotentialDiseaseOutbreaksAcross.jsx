...
// Feature: Outbreak detection flagging potential disease outbreaks across patient population
...
export default function CfOutbreakDetectionFlaggingPotentialDiseaseOutbreaksAcross() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-outbreak-detection-flagging-potential-disease-outbreaks-across-patient/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-outbreak-detection-flagging-potential-disease-outbreaks-across-patient', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-outbreak-detection-flagging-potential-disease-outbreaks-across-patient/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Outbreak detection flagging potential disease outbreaks across patient population</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVeterinaryClinicManager</p>
      </div>
