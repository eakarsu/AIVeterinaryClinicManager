...
export default function GapLimitedPharmacySystemIntegrationOnlyStubModules() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-pharmacy-system-integration-only-stub-modules/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-pharmacy-system-integration-only-stub-modules', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-pharmacy-system-integration-only-stub-modules/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited pharmacy system integration (only stub modules)</h1>
