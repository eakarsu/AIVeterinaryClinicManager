...
// Feature: Diagnostic assistant providing differential diagnoses from symptoms/labs
...
export default function CfDiagnosticAssistantProvidingDifferentialDiagnosesFromSymptoms() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-diagnostic-assistant-providing-differential-diagnoses-from-symptoms-labs/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-diagnostic-assistant-providing-differential-diagnoses-from-symptoms-labs', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-diagnostic-assistant-providing-differential-diagnoses-from-symptoms-labs/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Diagnostic assistant providing differential diagnoses from symptoms/labs</h1>
