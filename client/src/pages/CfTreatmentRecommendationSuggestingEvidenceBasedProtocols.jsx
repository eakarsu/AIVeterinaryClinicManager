...
// Feature: Treatment recommendation suggesting evidence-based protocols
...
export default function CfTreatmentRecommendationSuggestingEvidenceBasedProtocols() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-treatment-recommendation-suggesting-evidence-based-protocols/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-treatment-recommendation-suggesting-evidence-based-protocols', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-treatment-recommendation-suggesting-evidence-based-protocols/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Treatment recommendation suggesting evidence-based protocols</h1>
