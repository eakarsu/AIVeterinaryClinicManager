...
export default function CfAftercareGeneratorProducingDischargeInstructionsAndFollow() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-aftercare-generator-producing-discharge-instructions-and-follow-up-plans/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-aftercare-generator-producing-discharge-instructions-and-follow-up-plans', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-aftercare-generator-producing-discharge-instructions-and-follow-up-plans/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Aftercare generator producing discharge instructions and follow-up plans</h1>
