...
export default function GapNoLabResultDirectIntegrationIdexxAntech() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-lab-result-direct-integration-idexx-antech/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-lab-result-direct-integration-idexx-antech', project: 'AIVeterinaryClinicManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-lab-result-direct-integration-idexx-antech/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No lab result direct integration (IDEXX, Antech)</h1>
