...
// Diagnostic assistant providing differential diagnoses from symptoms/labs
// Feature: diagnostic-assistant
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'diagnostic-assistant', payload, result);
    res.json({ success: true, feature: 'diagnostic-assistant', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Diagnostic assistant providing differential diagnoses from symptoms/labs. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'diagnostic-assistant', title: 'Diagnostic assistant providing differential diagnoses from symptoms/labs', project: 'AIVeterinaryClinicManager' });
});
