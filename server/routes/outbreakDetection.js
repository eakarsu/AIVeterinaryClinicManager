...
// Outbreak detection flagging potential disease outbreaks across patient population
// Feature: outbreak-detection
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'outbreak-detection', payload, result);
    res.json({ success: true, feature: 'outbreak-detection', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Outbreak detection flagging potential disease outbreaks across patient population. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'outbreak-detection', title: 'Outbreak detection flagging potential disease outbreaks across patient population', project: 'AIVeterinaryClinicManager' });
});
