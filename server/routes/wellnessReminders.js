...
// Wellness reminder automation by species/age
// Feature: wellness-reminders
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'wellness-reminders', payload, result);
    res.json({ success: true, feature: 'wellness-reminders', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Wellness reminder automation by species/age. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'wellness-reminders', title: 'Wellness reminder automation by species/age', project: 'AIVeterinaryClinicManager' });
});
