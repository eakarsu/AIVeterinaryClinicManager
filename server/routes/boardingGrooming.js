...
// Boarding/grooming module add-on for multi-service clinics
// Feature: boarding-grooming
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'boarding-grooming', payload, result);
    res.json({ success: true, feature: 'boarding-grooming', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Boarding/grooming module add-on for multi-service clinics. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'boarding-grooming', title: 'Boarding/grooming module add-on for multi-service clinics', project: 'AIVeterinaryClinicManager' });
});
