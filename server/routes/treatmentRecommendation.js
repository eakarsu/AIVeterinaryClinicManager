...
// Treatment recommendation suggesting evidence-based protocols
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'treatment-recommendation', payload, result);
    res.json({ success: true, feature: 'treatment-recommendation', ...result });
  } catch (err) {
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Treatment recommendation suggesting evidence-based protocols. Batch of ${items.length} items: ${JSON.stringify(items).slice(0, 3500)}.\nReturn JSON: { results:[], aggregate:{} }`,
      { maxTokens: 3072 }
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'treatment-recommendation', title: 'Treatment recommendation suggesting evidence-based protocols', project: 'AIVeterinaryClinicManager' });
});
